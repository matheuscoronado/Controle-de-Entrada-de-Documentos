<?php $__env->startSection('title', 'Processos'); ?>
<?php $__env->startSection('subtitle', 'Consulta e acompanhamento de processos abertos'); ?>

<?php $__env->startSection('topbar-actions'); ?>
    <a href="<?php echo e(route('documentos.create')); ?>" class="btn-primary-sced">
        ➕ Novo Processo
    </a>
<?php $__env->stopSection(); ?>

<?php $__env->startSection('content'); ?>


<div class="filtros-card">
    <form method="GET" action="<?php echo e(route('documentos.index')); ?>">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;">
            <span style="font-size:14px;font-weight:600;color:var(--azul-escuro);">🔍 Filtros</span>
            <?php if(request()->anyFilled(['protocolo','remetente','tipo_documento_id','status','data_inicio','data_fim'])): ?>
                <a href="<?php echo e(route('documentos.index')); ?>" class="btn-secondary-sced"
                   style="padding:4px 10px;font-size:12px;">✕ Limpar</a>
            <?php endif; ?>
        </div>
        <div class="row g-2">
            <div class="col-12 col-md-2">
                <label>Protocolo</label>
                <input type="text" name="protocolo" class="form-input-sced"
                       placeholder="2026-000001" value="<?php echo e(request('protocolo')); ?>">
            </div>
            <div class="col-12 col-md-3">
                <label>Solicitante</label>
                <input type="text" name="remetente" class="form-input-sced"
                       placeholder="Nome do solicitante" value="<?php echo e(request('remetente')); ?>">
            </div>
            <div class="col-12 col-md-2">
                <label>Serviço</label>
                <select name="tipo_documento_id" class="form-input-sced">
                    <option value="">Todos</option>
                    <?php $__currentLoopData = $tipos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tipo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($tipo->id); ?>" <?php echo e(request('tipo_documento_id') == $tipo->id ? 'selected' : ''); ?>>
                            <?php echo e($tipo->nome); ?>

                        </option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </div>
            <div class="col-12 col-md-2">
                <label>Status</label>
                <select name="status" class="form-input-sced">
                    <option value="">Todos</option>
                    <option value="novo"        <?php echo e(request('status') == 'novo'        ? 'selected':''); ?>>Novo</option>
                    <option value="recebido"    <?php echo e(request('status') == 'recebido'    ? 'selected':''); ?>>Recebido</option>
                    <option value="em_analise"  <?php echo e(request('status') == 'em_analise'  ? 'selected':''); ?>>Em Análise</option>
                    <option value="encaminhado" <?php echo e(request('status') == 'encaminhado' ? 'selected':''); ?>>Encaminhado</option>
                    <option value="finalizado"  <?php echo e(request('status') == 'finalizado'  ? 'selected':''); ?>>Finalizado</option>
                </select>
            </div>
            <div class="col-6 col-md-1">
                <label>De</label>
                <input type="date" name="data_inicio" class="form-input-sced" value="<?php echo e(request('data_inicio')); ?>">
            </div>
            <div class="col-6 col-md-1">
                <label>Até</label>
                <input type="date" name="data_fim" class="form-input-sced" value="<?php echo e(request('data_fim')); ?>">
            </div>
            <div class="col-12 col-md-1" style="display:flex;align-items:flex-end;">
                <button type="submit" class="btn-primary-sced" style="width:100%;">Buscar</button>
            </div>
        </div>
    </form>
</div>


<div class="card-sced">
    <div style="overflow-x:auto;">
        <table class="tabela-sced">
            <thead>
                <tr>
                    <th>Protocolo</th>
                    <th>Serviço</th>
                    <th>Solicitante</th>
                    <th>Setor Destino</th>
                    <th>Abertura</th>
                    <th>Anexos</th>
                    <th>Status</th>
                    <th style="text-align:center;">Ação</th>
                </tr>
            </thead>
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $documentos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $doc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr>
                    <td>
                        <span class="protocolo-codigo"><?php echo e($doc->numero_protocolo); ?></span>
                    </td>
                    <td style="font-weight:600;font-size:13px;"><?php echo e($doc->tipoDocumento->nome); ?></td>
                    <td style="font-size:13px;"><?php echo e($doc->remetente); ?></td>
                    <td style="font-size:13px;color:var(--cinza-600);"><?php echo e($doc->setor_destino); ?></td>
                    <td style="font-size:13px;color:var(--cinza-600);white-space:nowrap;">
                        <?php echo e(\Carbon\Carbon::parse($doc->data_recebimento)->format('d/m/Y')); ?>

                    </td>
                    <td style="text-align:center;">
                        <?php $nAnexos = $doc->anexos_count ?? 0; ?>
                        <?php if($nAnexos > 0): ?>
                            <span style="font-size:12px;font-weight:600;color:var(--azul-claro);">
                                📎 <?php echo e($nAnexos); ?>

                            </span>
                        <?php else: ?>
                            <span style="color:var(--cinza-400);font-size:12px;">—</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <span class="badge-status badge-<?php echo e($doc->status); ?>">
                            <?php echo e([
                                'novo'        => 'Novo',
                                'recebido'    => 'Recebido',
                                'em_analise'  => 'Em Análise',
                                'encaminhado' => 'Encaminhado',
                                'finalizado'  => 'Finalizado'
                            ][$doc->status] ?? ucfirst($doc->status)); ?>

                        </span>
                    </td>
                    <td style="text-align:center;">
                        <a href="<?php echo e(route('documentos.show', $doc)); ?>"
                           class="btn-outline-sced" style="font-size:12px;padding:5px 12px;">
                            Ver
                        </a>
                    </td>
                </tr>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                <tr>
                    <td colspan="8" style="text-align:center;padding:48px;color:var(--cinza-400);">
                        <div style="font-size:32px;margin-bottom:8px;">📂</div>
                        Nenhum processo encontrado.<br>
                        <a href="<?php echo e(route('documentos.create')); ?>" style="color:var(--azul-claro);font-weight:600;">
                            Abrir o primeiro processo →
                        </a>
                    </td>
                </tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>

    <?php if($documentos->hasPages()): ?>
    <div style="padding:16px 24px;border-top:1px solid var(--cinza-200);">
        <?php echo e($documentos->withQueryString()->links()); ?>

    </div>
    <?php endif; ?>
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Controle-de-Entrada-de-Documentos-main\sced\resources\views/processos/index.blade.php ENDPATH**/ ?>